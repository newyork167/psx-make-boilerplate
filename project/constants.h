...
Coords clampBounds(Image image, int x, int y) {
	if (x < 0) {
		x = 0;
	} else if (x + image.sprite.w > SCREEN_WIDTH) {
		x = SCREEN_WIDTH - image.sprite.w;
	}

	if (y < 0) {
		y = 0;
	} else if (y + image.sprite.h >= SCREEN_HEIGHT) {
		y = SCREEN_HEIGHT - image.sprite.h;
	}
...
Image moveImage(Image image, int x, int y) {
	Coords coords = clampBounds(image, x, y);
	image.sprite.x = coords.x;
	image.sprite.y = coords.y;
	return image;
}
